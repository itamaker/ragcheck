...
1. Create a GitHub repository named `ragcheck`.
...
```bash
./scripts/render-homebrew-formula.sh --owner itamaker --version v0.1.0 > /path/to/homebrew-tap/Formula/ragcheck.rb
```

Commit the rendered file to `https://github.com/itamaker/homebrew-tap` as `Formula/ragcheck.rb`, then users can install with:
